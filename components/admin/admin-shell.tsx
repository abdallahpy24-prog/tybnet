...
import { Activity, CalendarDays, FileText, FlaskConical, Home, LayoutDashboard, Map, MapPin, Pill, Settings, Shield, Stethoscope, Tags, Users } from "lucide-react";
    ...
    <div className="min-h-screen bg-surface lg:grid lg:grid-cols-[300px_1fr]">
      <aside className="border-l border-borderSoft bg-white p-5 lg:min-h-screen">
        <div className="mb-8 flex items-center gap-3">
          <Image src="/assets/logo.png" alt="طب نت" width={56} height={54} className="h-14 w-14 object-contain" />
          <div>
            <p className="text-lg font-black text-navy">لوحة طب نت</p>
            <p className="text-xs font-bold text-primary-dark">Abdullah Hamed</p>
          </div>
        </div>
        <nav className="grid gap-2">
          {nav.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold text-slate-600 hover:bg-primary-soft hover:text-primary-dark">
        ...
        <div className="mt-8 grid gap-2">
          <Link href="/" className="flex items-center gap-2 rounded-xl px-3 py-3 text-sm font-bold text-slate-600 hover:bg-primary-soft">
