...
export default async function ProviderDetailsPage({
  ...
  const name = displayName(provider);
  const listPath = provider.type === "DENTIST" ? "/dentists" : "/doctors";
  const backHref = safeReturnPath(query.from, listPath);
  const backLabel = provider.type === "DENTIST" ? "العودة إلى نتائج أطباء الأسنان" : "العودة إلى نتائج الأطباء";
  ...
  const canonicalUrl = `${getSiteUrl()}/providers/${provider.slug}`;
  const structuredData = {
    "@context": "https://schema.org",
    "@type": provider.type === "DENTIST" ? "Dentist" : "Physician",
    name,
    url: canonicalUrl,
